...
NSString* _accountIdentifier_key = @"286acf8b16bd4803abf9c7344ca39c16";

int64_t _bannerPlacementId_slot = 10000445873;

int64_t _interstitialPlacementId_page = 10000445864;

int64_t _rewardedVideoPlacementId_video = 10000445875;

+ (void) _setupAdPlatform_init
{
    void (^_completionHandler_block)(NSError*) = ^( NSError* _Nullable  error) {
        if (error) {
    ...
    [IMSdk initWithAccountID:_accountIdentifier_key consentDictionary:@{IMCommonConstants.IM_GDPR_CONSENT_AVAILABLE: @"true", IMCommonConstants.IM_GDPR_CONSENT_IAB: @"<<consent format="" iab="" in="">>"} andCompletionHandler:_completionHandler_block];
}
...
+ (void)_showRewardedVideoAd_play {
    ...
    if (rootVC && _rewardedVideoInstance_media.isReady) {
...
+ (void)_presentInterstitialAd_display{
    if([self _checkTimeIntervalExceeds15s_validate])
    {
        ...
        if (rootVC && _interstitialAdInstance_page.isReady) {
...
+ (void)_configureContainerLayout_setup {
    ...
    if (!rootVC) {
        NSLog(@"无法获取rootViewController，跳过容器布局设置");
        return;
    }
